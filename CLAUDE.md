...
| Node 순수 PDF 렌더 라이브러리 선택 | 🔵 `pdfjs-dist` 6.3.289(Apache-2.0) + `@napi-rs/canvas` 1.0.9(MIT, 번들 Skia BSD-3-Clause) (2026-09-21, S2-T0). Windows 11·Node 24 에서 빌드 도구 없이 설치, `npm ci --ignore-scripts` 후에도 렌더 동작, 한글 1쪽 → 1272x1800 PNG 약 0.1초, PNG/JPG 리사이즈도 같은 라이브러리로 가능. 탈락: `mupdf`(AGPL), `sharp`(`@img/sharp-win32-x64` 가 LGPL-3.0-or-later 포함). 🟡 폰트 미임베드 한글 PDF 는 미검증(T4 에서 확인) | 출처: `npm view <pkg> version license`, https://github.com/mozilla/pdf.js/blob/master/examples/node/pdf2png/pdf2png.mjs , https://github.com/Brooooooklyn/canvas , https://github.com/google/skia/blob/main/LICENSE |
...
| `disable-model-invocation` 스킬의 always-on 토큰 | 🟡 `plugin details` 는 `init-exam` 에 ~110 tok 를 매기나(2026-09-21, 합계 ~325), skills 문서는 description 이 컨텍스트에 들어가지 않는다고 함. 어느 쪽이 맞는지 미확인 | 실제 세션의 시스템 프롬프트/스킬 목록에 `init-exam` 이 보이는지 확인 |
